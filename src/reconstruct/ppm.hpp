...
void PPM4_subView(const SubView& Q_, Real& ql_ip1, Real& qr_i){
  static_assert(SubView::rank == 1, "expect (5, Ni) subview");
  // Q_ is a subview of a 5-element array containing the values at i-2, i-1, i, i+1, and i+2
  Real q_im2 = Q_(0);
  Real q_im1 = Q_(1);
  Real q_i   = Q_(2);
  Real q_ip1 = Q_(3);
...
template <class SubView>
KOKKOS_INLINE_FUNCTION
void PPMX_subView(const SubView& Q_, Real& ql_ip1, Real& qr_i){
  static_assert(SubView::rank == 1, "expect (5, Ni) subview");
  Real q_im2 = Q_(0);
  Real q_im1 = Q_(1);
  Real q_i   = Q_(2);
...
KOKKOS_INLINE_FUNCTION
void PiecewiseParabolicX3_old(TeamMember_t const &member,
     const EOS_Data &eos, const bool extremum_preserving, const bool apply_floors,
     const int m, const int k, const int j, const int il, const int iu,
...
//----------------subview version of PiecewiseParabolicX3
...
void PiecewiseParabolicX3(TeamMember_t const &member,
  ...
  for(int n=0; n<nvar; ++n) {
    ...
    auto Q_ = Kokkos::subview(q, m, n, Kokkos::make_pair(k-2, k+3), j,Kokkos::ALL());  
    ...
    par_for_inner(member, il, iu, [&](const int i){
      ...
      if(extremum_preserving)
      {
        ...
        //
        PPM4_subView(window,ql_kp1(n,i),qr_k(n,i));
        //PPM4(qkm2, qkm1, qk, qkp1, qkp2, ql_kp1(n,i), qr_k(n,i));
      }
    });
