...
void SingleStateFluxLR(const PrimitiveSolverHydro<EOSPolicy, ErrorPolicy>& eos,
    const int nmhd, const int nscal,
    Real g3d[NSPMETRIC], Real beta_u[3], Real alpha,
    Real prim_l[NPRIM], Real Bu_l[NPRIM] ,  Real cons_l[NCONS],
    Real flux_l[NCONS], Real bflux_l[NMAG], Real& bsql
){
  constexpr int ibx = ivx - IVX;
  constexpr int iby =(ibx + 1)%3;
  constexpr int ibz =(ibx + 2)%3;
  constexpr int pvx = PVX + ibx;
  constexpr int pvy = PVX + iby;
  constexpr int pvz = PVX + ibz;
  constexpr int csx = CSX + ibx;
  ...
  bflux_l[ibx]=  0.0;
  bflux_l[iby]= (Bu_l[iby]*vcl - Bu_l[ibx]*(prim_l[pvy]*iWl - beta_u[pvy - PVX]*ialpha));
  bflux_l[ibz]= (Bu_l[ibz]*vcl - Bu_l[ibx]*(prim_l[pvz]*iWl - beta_u[pvz - PVX]*ialpha));
}
...
void SingleStateFlux(const PrimitiveSolverHydro<EOSPolicy, ErrorPolicy>& eos,
  ...
  bflux_r[ibx] = 0.0;
  bflux_r[iby] = (Bu_r[iby]*vcr - Bu_r[ibx]*(prim_r[pvy]*iWr - beta_u[pvy - PVX]*ialpha));
  bflux_r[ibz] = (Bu_r[ibz]*vcr - Bu_r[ibx]*(prim_r[pvz]*iWr - beta_u[pvz - PVX]*ialpha));
}
